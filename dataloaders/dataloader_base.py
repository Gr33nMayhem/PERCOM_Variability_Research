...
class BASE_DATA():
        ...
        for sub_id in df.index.unique():
            ...
            for col in temp.columns:
                ...
                if 'acc' in col and "diff" not in col:
                    ...
                    grav_acc, body_acc = components_selection_one_signal(t_signal, freq1, freq2,
                                                                         self.freq)  # apply components selection
                    ...
                    _, body_gyro = components_selection_one_signal(t_signal, freq1, freq2,
                                                                   self.freq)  # apply components selection
    ...
    def genarate_spectrogram(self, flag="train"):
        ...
        if flag == "train":
            ...
            self.train_freq_path = os.path.join(save_path,
                                                "diff_{}_window_{}_step_{}_drop_trans_{}".format(self.difference,
                                                                                                 self.windowsize,
                                                                                                 displacement,
                                                                                                 self.drop_transition))
            ...
            self.test_freq_path = os.path.join(save_path,
                                               "diff_{}_window_{}_step_{}_drop_trans_{}".format(self.difference,
                                                                                                self.windowsize,
                                                                                                displacement,
                                                                                                self.drop_transition))
            ...
            for window in slidingwindows:
                ...
                for j in range(sample_x.shape[1]):
                    if self.difference and j >= int(sample_x.shape[1] / 2):
                        [cwtmatr, frequencies] = pywt.cwt(sample_x[:, j], scales2, self.wavename,
                                                          sampling_period=1.0 / self.freq)  # 连续小波变换模块
                    else:
                        [cwtmatr, frequencies] = pywt.cwt(sample_x[:, j], scales1, self.wavename,
                                                          sampling_period=1.0 / self.freq)  # 连续小波变换模块
                    if resize_flag:
                        cwtmatr = resize(cwtmatr, (int(self.freq / f_scale), int(self.windowsize / l_scale)),
                                         mode='constant')
